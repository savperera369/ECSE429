Feature: Create a new To-do item
	...
	Scenario Outline: Normal flow - Successfully create a new to-do item.
		...
		And The response should include the generated ID "<id>" and the title, doneStatus, and the description I provided
	...
	Scenario Outline: Alternate flow - Successfully create a new to-do item with only a valid title
		...
		Then I should receive a "201 Created" status
		And The response should include the the generated ID "<id>", title I provided, and default values false and empty for the doneStatus and description respectively
