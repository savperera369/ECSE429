...
Feature: To-Do Project Association
	...
	Scenario Outline: Normal flow - Successfully create a to-do and project relationship
		...
		And the relationship should be established

	Scenario Outline: Alternate flow - To-do item already assigned to the project
		Given I have an existing to-do ID "<todo_id>" already related to project ID "<project_id>"
		When I send a POST request to "/todos/<todo_id>/tasksof" with project ID "<project_id>" in the body
		Then I should receive a "201 Created" status

	Scenario Outline: Error flow - Non-existent to-do or project ID
